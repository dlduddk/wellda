...
class Chatbot_:
    def __init__(self):
        ...
        #4. CONTEXT만으로 답변을 생성할 수 없고, 사용자의 정보가 답변 생성에 필요하다면 반드시 "~한 이유는 여러 가지가 있을 수 있어요. 예상하시는 추가정보를 주실 수 있을까요?" 와 같이 답하라: (예시: "혈압이 오른 이유는 여러 가지가 있을 수 있어요. 예상하시는 추가정보를 주실 수 있을까요?", "소화가 잘 안되는 이유는 여러 가지가 있을 수 있어요. 예상하시는 추가정보를 주실 수 있을까요?")
